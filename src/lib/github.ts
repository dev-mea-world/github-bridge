...
export async function getOctokit(): Promise<Octokit> {
  ...
  return new Octokit({ auth: (installation as any).token });
}

export async function getFileUtf8(params: {
  repoFull: string;
  path: string;
  ref?: string;
}) {
...
export async function createBranch(
  ...
  const { owner, repo } = parseRepo(repoFull);
  const octokit = await getOctokit();
  const from = await octokit.git.getRef({
    owner,
    repo,
    ref: `heads/${fromRef}`,
  });
...
export async function putFileUtf8(params: {
  ...
  const {
    ...
    path,
    content,
    message = "chore: update via Glor.IA",
    baseRef = "main",
  } = params;
  let { branch } = params;
  const { owner, repo } = parseRepo(repoFull);
...
export async function commentIssue(
  repoFull: string,
  params: { number: number; body: string }
) {
  const { owner, repo } = parseRepo(repoFull);
  const octokit = await getOctokit();
...
export async function addLabels(
  repoFull: string,
  params: { number: number; labels: string[] }
) {
  const { owner, repo } = parseRepo(repoFull);
  const octokit = await getOctokit();
